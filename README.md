...
wget http://big.hanyang.ac.kr/ETCHING/etching_v1.3.5.tar

# Load the image
docker load -i etching_v1.3.5.tar
...
|:---|:---|:---|:---|:---|
|etching|1.3.5|16647cac9a99|40 hours ago|4.3 GB|
...
```
docker run -i -t --rm -v /path/to/DEMO/:/work/ etching:1.3.5 etching \
...
```
Here, ```etching:1.3.5``` is ```REPOSITORY``` and ```TAG``` of ETCHING docker image.
...
```
docker run -i -t --rm -v /path/to/DEMO/:/work/ etching:1.3.5 /bin/bash
